...
// @grant        unsafeWindow
// @run-at       document-end
// @downloadURL https://github.com/dae2321/--Super-new-Xuexitong-course-brushing--/raw/main/index.js
// @updateURL https://github.com/dae2321/--Super-new-Xuexitong-course-brushing--/raw/main/index.js
// ==/UserScript==
...
(async function (vue, pinia$1, ElementPlus, md5, $$1) {
    ...
    function generateLLMPrompt(questionData) {
        ...
        if (questionData.type === "0") {
            ...
            prompt += `   - 如果选项包含特殊格式（如化学式下标），保留原格式输出\n\n`;
            ...
            prompt += `✅ 特殊示例（选项包含化学式等特殊格式）：\n`;
            prompt += `   题目：植物化石的气孔特征可以作为判别古大气( )浓度的生物指标。\n`;
            prompt += `   【选项列表】\n`;
            prompt += `   A. O <sub>2\n`;
            prompt += `   B. N <sub>2\n`;
            prompt += `   C. CO <sub>2\n`;
            prompt += `   D. He\n`;
            prompt += `   答案是A，你应该输出：O <sub>2\n\n`;
            prompt += `【你的答案】：`;
    ...
    class ServerApi {
        ...
        async defaultRequest(url, method, data = {}, headers = {}, type = false, timeout = 10000) {
            var _a;
            return type && (headers = { "Content-Type": "POST" == method ? "application/json" : "text/plain", Referer: this.windowz.location.href, v: _GM_info.script.version, ...headers }), withRetry(() => new Promise((resolve, reject) => {
                _GM_xmlhttpRequest({
                    ...
                    }, ontimeout: () => reject(new Error("Request timeout")), onerror: (err) => reject(new Error(`Network error`))
                });
                                    ...
                                    if (questionData.type === "0" || !answer.includes("#")) {
                                        ...
                                        if (!letterMatch) {
                                            ...
                                            for (let i = 0; i < questionData.options.length; i++) {
                                                if (answer === questionData.options[i]) {
                                                    console.log(`🤖 [大模型] 答案直接匹配选项: ${answer}`);
                                                    directMatch = true;
                                                    break;
                                                }
                                            }

                                            // 只有在不直接匹配选项的情况下，才尝试提取字母
                                            if (!directMatch) {
    ...
    class Cx {
        ...
        async audio(iframeWindow) {
            this.askStore.reset(), this.askStore.task.name = "音频";
            ...
            this.askStore.log(`音频开始播放 @ ${audioSpeed}倍速`, "info");
            ...
            return audio.play().then(function () {
                ...
                const intervalId = setInterval(() => {
                    if (Date.now() - audioStartTime > maxAudioDuration) {
                        ...
                        this.askStore.log("音频播放超时，自动继续", "warning");
                        resolve();
                        return;
                    }
                    ...
                    if (duration > 0) {
                        ...
                        this.askStore.task.status = `🎵 正在播放音频 ${percent}%`;
                    }
                    if (audio.ended) {
                        clearInterval(intervalId);
                        this.askStore.log("音频播放完成", "success");
                        resolve();
        ...
        async video(iframeWindow) {
            ...
            const detectPopupQuestion = (win) => {
                        ...
                        for (const tkTopic of tkTopics) {
                            if (tkTopic && tkTopic.offsetParent !== null) {
                                ...
                                if (tkTitle && tkOptions) {
                                    this.askStore.log(`检测到弹题`, "success");
                ...
                const selectors = [
                    ".testPop", ".popBox", ".questionDiv", ".layui-layer", ".ext-quiz-popup",
                    "[class*='question'][class*='popup']", "[class*='quiz'][class*='popup']",
                    ".ans-cc", ".ans-job-finish", ".jobFinish", ".layer", ".popups",
                    ".dialog", ".modal", "[class*='pop'][class*='test']", "[class*='exam'][class*='pop']",
                    ".xubox_layer", ".layui-layer-shade", ".popDiv", ".test_pop", ".videoQuiz",
                    ".quiz-pop", ".ans-insert-quiz"
                ];
                        ...
                        for (const element of elements) {
                            if (element && element.offsetParent !== null) {
                                ...
                                const questionElement = element.querySelector(".TiMu") ||
                                    element.querySelector(".fontLabel") || element.querySelector(".Zy_TItle") ||
                                    element.querySelector(".mark_name") || element.querySelector("[class*='title']") ||
                                    element.querySelector("[class*='question']") || element.querySelector(".afterHtml");
                                if (questionElement) {
            ...
            const detectPopupQuestionInAllWindows = () => {
                let result = detectPopupQuestion(iframeWindow);
                ...
                try {
                    if (iframeWindow.parent && iframeWindow.parent.document) {
            ...
            const handlePopupQuestion = async (questionInfo) => {
                ...
                try {
                    ...
                    if (questionElements.length === 0) {
                        try {
                            if (questionWin && questionWin.document) {
                    ...
                    for (let i = 0; i < questionElements.length; i++) {
                        ...
                        try {
                            ...
                            if (tkTitle && tkOptions) {
                                ...
                                if (questionType === "0" && (questionText.startsWith("多选题") || questionText.includes("【多选题】") || questionText.includes("（多选题）"))) {
                                    questionType = "1";
                            ...
                            if (!questionData || !questionData.question) {
                                ...
                                options.forEach(opt => {
                                    ...
                                    if (isValidOption) optionTexts.push(text);
                                });
                                ...
                                if (tkTopicTitle2) {
                                    ...
                                    if (titleText.includes("多选题")) questionType2 = "1";
                                    else if (titleText.includes("判断题")) questionType2 = "3";
                                }
            ...
            await new Promise((resolve) => {
                this.askStore.log("开始视频播放", "info");

                const handleVisibilityChange = () => {
                    const config = getConfig();
                    if (document.hidden && config.enableBackground && player.paused()) {
                        player.play();
                ...
                const intervalId = window.videoIntervalId = setInterval(async () => {
                    if (isProcessingPopupQuestion) {
                        ...
                        if (duration > 0) {
                            const percent = Math.floor((currentTime / duration) * 100);
                            this.askStore.task.status = `弹题处理中 ${percent}%`;
                        }
                        return;
                    }

                    if (Date.now() - videoStartTime > maxVideoDuration) {
                        this.askStore.log("视频播放超时，自动继续", "warning");
                        clearInterval(intervalId);
                    ...
                    if (duration && duration > 0) {
                        const percent = Math.floor((currentTime / duration) * 100);
                        this.askStore.task.status = `正在观看视频 ${percent}% @ ${videoSpeed}x`;
                    ...
                    if (popupQuestion) {
                        this.askStore.log("检测到弹题，正在处理...", "info");
                        await sleep(1);
                    ...
                    if ("isUnFinishJob" in iframeWindow && iframeWindow.isUnFinishJob()) {
                        if (player.paused() && !isProcessingPopupQuestion) {
                            const hasPopup = detectPopupQuestionInAllWindows();
                        ...
                        if (!isProcessingPopupQuestion) {
                            if (duration > 0 && currentTime >= duration - 1) {
                                ...
                                try {
                                    const jobIcon = iframeWindow.frameElement?.closest('.ans-job-icon')?.parentElement;
                                    if (jobIcon && jobIcon.classList.contains('ans-job-finished')) {
                                        clearInterval(intervalId);
                                        document.removeEventListener('visibilitychange', handleVisibilityChange);
                                        this.askStore.task.status = "视频播放完成";
                                        this.askStore.log("🎬 视频任务点已完成", "success");
                                        resolve();
                                    }
                                } catch (e) { }
                            }
                ...
                player.on("error", () => {
                    this.askStore.log("视频播放错误，等待恢复", "warning");
                    setTimeout(() => {
                        if (player.error()) {
                            this.askStore.log("视频错误无法恢复", "warning");
                            clearInterval(intervalId);
                            document.removeEventListener('visibilitychange', handleVisibilityChange);
                            resolve();
                        }
        ...
        case "/mooc2-ans/mycourse/studentstudy":
            ...
            const startWork = async () => {
                ...
                for (let i = 0; i < jobList.length; i++) {
                    ...
                    try {
                        ...
                        cxModel.askStore.log(`正在完成任务: ${otherInfo.name || otherInfo.title}`);

                        if (iframe.src.match(/\/ananas\/modules\/video\/index\.html/)) {
                            if (!cxModel.defaultConfig.autoVideo) {
                                cxModel.askStore.log("视频任务已跳过", "success");
                                continue;
                            }
                            await cxModel.video(iframe.contentWindow);
                            await sleep(2);
                            if (!parentEl?.classList.contains("ans-job-finished")) {
                                await sleep(3);
                            }
                            cxModel.askStore.log("视频任务已完成", "success");
                        } else if (iframe.src.match(/\/ananas\/modules\/work\/index.html/)) {
                            const taskName = otherInfo.name || otherInfo.title || "";
                            const property = otherInfo.property || {};
                            let isExam = taskName.includes("测验") || taskName.includes("考试") || taskName.includes("测试") ||
                                property.exam === true || property.type === "exam" || iframe.src.includes("exam");

                            const workIframe = iframe.contentWindow?.document?.querySelector("iframe");
                            if (workIframe && workIframe.contentWindow) {
                                await waitIframeLoaded(workIframe);
                                cxModel.askStore.log(isExam ? "开始章节测验" : "开始作业", "info");
                                await cxModel.work(workIframe.contentWindow, isExam);
                                await sleep(2);
                                cxModel.askStore.log(isExam ? "章节测验完成" : "作业完成", "success");
                            } else {
                                cxModel.askStore.log("未找到作业iframe，跳过", "warning");
                            }
                        } else if (iframe.src.match(/\/ananas\/modules\/audio\/index.html/)) {
                            if (!cxModel.defaultConfig.autoVideo) continue;
                            await cxModel.audio(iframe.contentWindow);
                            await sleep(2);
                            cxModel.askStore.log("音频任务已完成", "success");
                        } else if (iframe.src.match(/\/ananas\/modules\/pdf\/index.html/)) {
                            await cxModel.pdf(iframe.contentWindow);
                            await sleep(2);
                            cxModel.askStore.log("pdf任务已完成", "success");
                        } else {
                            cxModel.askStore.log(`未知任务类型，跳过`, "warning");
                        }
                    } catch (e) {
                        cxModel.askStore.log("处理当前任务点失败，已跳过: " + e.message, "error");
                    }
                }

                if (thisTaskId !== _globalTaskId) {
                    cxModel.askStore.log("检测到章节切换，取消后续操作", "warning");
                    return;
                }
                ...
                if (thisTaskId !== _globalTaskId) {
                    cxModel.askStore.log("检测到章节切换，取消自动跳转", "warning");
                    return;
                }

                if (_justClickedNext) {
                    cxModel.askStore.log("已点击过下一章按钮，等待页面跳转", "warning");
                    return;
                }
                ...
                try {
                    const nextBtn1 = top?.document.querySelector("#prevNextFocusNext");
                    const nextBtn2 = top?.document.querySelector(".jb_btn.jb_btn_92.fr.fs14.nextChapter");
                    const nextBtn3 = top?.document.querySelector("#nextBtn");
                    const nextBtn4 = top?.document.querySelector(".nextChapter");
                    ...
                    if (nextButton) {
                        ...
                        cxModel.askStore.log("切换到下一章节", "success");
                        nextButton.click();
                    } else {
                        cxModel.askStore.log("未找到下一章节按钮，可能是最后一章", "warning");
                    }
                } catch (e) {
                    cxModel.askStore.log("章节切换失败: " + e.message, "error");
                }
            };
            ...
            const checkUrlChange = async () => {
                try {
                    ...
                    const _self1 = cardsIframe.contentWindow;
                    if (!_self1) return;
                    const currentHref = _self1.location?.href;
                    if (!currentHref) return;

                    const srcChanged = currentHref !== _lastIframeSrc;
                    const now = Date.now();

                    if (srcChanged && (now - lastUrlChangeTime > 2000)) {
                        _lastIframeSrc = currentHref;
                        ...
                        if (cxModel?.askStore) {
                            ...
                            cxModel.askStore.log("章节切换，重新开始", "info");
                        }
                        startWork();
                    }
                } catch (e) {
